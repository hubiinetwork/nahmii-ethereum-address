...
describe('EthereumAddress', () => {
    ...
    context('a new EthereumAddress from null input', () => {
        it('returns null from factory function', () => {
            expect(EthereumAddress.from(null)).to.be.null;
        });
    });

    context('a new EthereumAddress from undefined input', () => {
        it('returns null from factory function', () => {
            expect(EthereumAddress.from(undefined)).to.be.null;
        });
    });

    describe('#assertEthereumAddress()', () => {

        context('EthereumAddress instance', () => {
            it('does not throw error', () => {
                const obj = EthereumAddress.from('0x473A47d4Ae5b86bc5897A8Af1c8BeA297a1399dC');
                assert.doesNotThrow(() => EthereumAddress.assertEthereumAddress(obj));
            });
        });

        context('string', () => {
            it('throws TypeError', () => {
                const obj = '0x473A47d4Ae5b86bc5897A8Af1c8BeA297a1399dC';
                assert.throws(() => EthereumAddress.assertEthereumAddress(obj), TypeError);
            });
